...
`GEMINI_API_KEY`가 없어도 선택된 영문 제목은 무료 MyMemory API로 번역하며, 텔레그램에는 **뉴스 1개당 게시물 1개**로 한국어 제목과 영문 원제를 함께 표시한다. 번역 호출이 실패하면 영문 원제로 안전하게 되돌아가므로 발송은 중단되지 않는다. 키를 넣으면 기사 묶음 전체를 한 번만 호출해 한국어 제목·요약·실무 포인트를 만든다.
